...
// Zustand store
const useSlumberlandStore = create((set, get) => ({
  // Data
  markets: mockMarkets,
  ...
  getFilteredMarkets: () => {
    ...
    return markets.filter(market => {
      if (filters.blueOceanOnly && !market.blueOcean) return false;
      if (market.opportunityScore < filters.minOpportunityScore) return false;
      if (filters.selectedCategories.length > 0 && !filters.selectedCategories.includes(market.category)) return false;
      return true;
    });
  },
  ...
  getFilteredInfluencers: () => {
    ...
    return influencers.filter(influencer => {
      if (filters.selectedPlatforms.length > 0 && !filters.selectedPlatforms.includes(influencer.platform)) return false;
      if (influencer.followers < filters.minFollowers) return false;
      if (influencer.engagement < filters.minEngagement) return false;
      if (filters.selectedNiches.length > 0 && !filters.selectedNiches.includes(influencer.niche)) return false;
      return true;
    });
  },

  getInfluencerById: (id) => {
    const { influencers } = get();
    return influencers.find(inf => inf.id === id);
  },

  getMarketById: (id) => {
    const { markets } = get();
    return markets.find(mkt => mkt.id === id);
  }
}));

export default useSlumberlandStore;
